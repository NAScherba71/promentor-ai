...
const CodingChallenge: React.FC<CodingChallengeProps> = ({ layoutHeight = 'full' }) => {
  ...
  const loadChallenge = async () => {
    // Try to use real backend first, unless explicitly disabled
    const forceMock = process.env.NEXT_PUBLIC_FORCE_MOCK === 'true'
    
    if (!forceMock) {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_AI_URL || 'http://localhost:5000'
        const response = await fetch(`${apiUrl}/challenges/generate`, {
          method: 'POST',
    ...
    try {
      ...
      if (!forceMock) {
        ...
        try {
          ...
          // Ensure we fall through to mock execution
        }
      }
